...
While @Id <= 15000
Begin 
   Insert Into dbo.Stores values (@Id, 'S'+ CAST(Next Value for Sq_Store_State as NVARCHAR(10)), 
...
While @Id <= 1000
Begin 
   Insert Into dbo.ProductCategory values (@Id, 'in category-'+ CAST(@Id as NVARCHAR(20)), 
...
While @Id <= 15000
Begin 
   Insert Into dbo.ProductSubCategory values (@Id, 'SubCategory' + CAST(@Id as NVARCHAR(15)), 
...
While @Id <= 400000
Begin 
   Insert Into dbo.Item values (@Id, 'Item' + CAST(@Id as NVARCHAR(15)), 'New', CAST(NEXT VALUE for Sq_ProductItem_Price as money),
   'described as-' + CAST(@Id as NVARCHAR(15)), DATEADD(day,-(NEXT VALUE for Sq_ProductItem_SubCategoryID),GETDATE()), NEXT VALUE for Sq_ProductItem_CustomerID,
...
While @Id <= 500000
Begin 
   Insert Into dbo.Item values (@Id, 'Item' + CAST(@Id as NVARCHAR(15)), 'Used', CAST(NEXT VALUE for Sq_ProductItem_Price as money),
   'described as-' + CAST(@Id as NVARCHAR(15)), DATEADD(day,-(NEXT VALUE for Sq_ProductItem_SubCategoryID),GETDATE()), NEXT VALUE for Sq_ProductItem_CustomerID,
   NEXT VALUE for Sq_Store_StoreID, NEXT VALUE for Sq_ProductItem_SubCategoryID)
...
-- set sold on date and customerID as NULL for few items as they are still in stock and not sold to any customer
Declare @Id int
set @Id = 1
...
Begin 
   ...
   NEXT VALUE For Sq_Customer_Age, 'S' + CAST(Next value for Sq_Store_State as NVARCHAR(20)), 'C' + CAST(Next Value for Sq_Store_City as NVARCHAR(20)))
...
as
    SELECT i.ItemID, i.CustomerID, s.[State], s.City, i.SoldOn, YEAR(i.SoldOn) as [YearSoldOn], MONTH(i.SoldOn) as [MonthSoldOn],
    ...
    [Age Group] =
        ...
        End
    from dbo.Item i, dbo.Customer c, dbo.Stores s, dbo.ProductCategory pc, dbo.ProductSubCategory ps
...
CREATE PROC sp_Add_Item @ItemID int, @Condition NVARCHAR(5), @Price money, @StoreID int, @SubCategoryID int
As
    ...
    Insert into dbo.Item VALUES (@ItemID, 'Item' + CAST(@ItemID as NVARCHAR(15)), @Condition, @Price, 'described as-' + CAST(@ItemID as NVARCHAR(20)), NULL, NULL, @StoreID, @SubCategoryID)
...
-- create procedure to update the status of the item to sold and update the 'sold on' date and enter the customer id of the customer who bought it
...
-- create procedure to update the status of the item to returned and update the 'sold on' date
-- raise error if item id is not in the given list
...
CREATE PROC sp_Add_Customer @CustomerID int, @CustomerName NVARCHAR(15), @Age int, @State NVARCHAR(15), @City NVARCHAR(15)
As
